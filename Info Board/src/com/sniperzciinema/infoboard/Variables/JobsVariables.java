...
public class JobsVariables {
	...
	public static String replaceVariables(String string, Player player) {
		...
		if (newString.contains("<jobsin>"))
			newString = newString.replaceAll("<jobsin>", String.valueOf(Jobs.getJobsDAO().getAllJobs(player).size()));
		
		if (newString.contains("<jobstitle"))
		{
			int i = Integer.valueOf(newString.split("<jobstitle")[1].split(">")[0]) - 1;
			
			if (Jobs.getJobsDAO().getAllJobs(player).size() >= (i + 1))
				newString = newString.replaceAll("<jobstitle" + (i + 1) + ">", String.valueOf(Jobs.getJobsDAO().getAllJobs(player).get(i).getJobName()));
			else
		...
		if (newString.contains("<jobslevel"))
		{
			...
			if (Jobs.getJobsDAO().getAllJobs(player).size() >= (i + 1))
				newString = newString.replaceAll("<jobslevel" + (i + 1) + ">", String.valueOf(Jobs.getJobsDAO().getAllJobs(player).get(i).getLevel()));
			else
		...
		if (newString.contains("<jobsexp"))
		{
			...
			if (Jobs.getJobsDAO().getAllJobs(player).size() >= (i + 1))
				newString = newString.replaceAll("<jobsexp" + (i + 1) + ">", String.valueOf(Jobs.getJobsDAO().getAllJobs(player).get(i).getExperience()));
			else
		...
		if (newString.contains("<jobsmaxlevel"))
		{
			...
			if (Jobs.getJobsDAO().getAllJobs(player).size() >= (i + 1))
				newString = newString.replaceAll("<jobsmaxlevel" + (i + 1) + ">", String.valueOf(Jobs.getJob(Jobs.getJobsDAO().getAllJobs(player).get(i).getJobName()).getMaxLevel()));
			else
