...
import me.xxsniperzzxxsd.infoboard.ScoreBoard;
...
public class ScrollText {
	...
	public boolean slideScore(Player player) {
		...
		if (plugin.ScrollManager.getScrollers(player) != null)
			if (!plugin.getConfig().getStringList("Disabled Worlds").contains(player.getWorld().getName()) && !ScoreBoard.hidefrom.contains(player.getName()) && (player.getScoreboard().getObjective(DisplaySlot.SIDEBAR) == null || player.getScoreboard().getObjective(DisplaySlot.SIDEBAR).getName().equalsIgnoreCase("InfoBoard")))
			{
